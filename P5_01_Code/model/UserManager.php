...
class UserManager extends AbstractManager
{
	...
	public function moderatAdminSorting($users)
	{
		if ($_SESSION['school'] === ALL_SCHOOL) {
			...
			$nbModerator = 0;
			foreach ($users as $user) {
				if ($user->getIsModerator()) {$nbModerator++;}
			}
	...
	public function signUp($POST, SchoolManager $SchoolManager, HistoryManager $HistoryManager)
	{
		if ($this->checkForScriptInsertion($_POST)) {
			if ($POST['confirmPassword'] === $POST['password']) {
						...
						if (!empty($POST['affiliationCode'])) {
							...
							if ($result['exist']) {
								...
								if ($school->getNbActiveAccount() < $school->getNbEleve()) {
									...
									$this->add(new User(['name' => $POST['pseudo'], 
														'mail' => $POST['mail'], 
							...
							} else {return $message = "Le code d'affiliation est incorrect";}
						} else {
				...
				} else {return $message = "Ce nom d'utilisateur est déja utilisé";}
			} else {return $message = "Vous devez entrer deux mot de passe identiques";}
		} else {$message = "Les informations renseignées sont incorrectes";}
	}
