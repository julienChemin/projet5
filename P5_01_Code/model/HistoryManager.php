...
class HistoryManager extends Database
{
	...
	public function getBySchool($school, int $offset = 0, string $sortBy = null, $sortValue = null, $secondSortValue = null)
	{
		if (intval($school) > 0) {
			if (empty($sortBy)) {
				//all school entries
				$q = $this->sql('SELECT category, entry, DATE_FORMAT(dateEntry, "%d/%m/%Y à %H:%m") AS dateEntry 
	...
	{
		$this->sql('INSERT INTO ' . static::$TABLE_NAME . ' (idSchool, category, entry, dateEntry) 
	...
	public function getSchoolHistory($GET)
	{
		...
		if (!empty($GET['sortBy']) && !empty($GET['sortValue'])) {
			if (!empty($GET['thirdSortValue'])) {
				...
				$entries = $this->getBySchool($GET['school'], 
														$offset, 
														$GET['sortBy'], 
														$GET['sortValue'], 
