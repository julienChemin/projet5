...
function addEventDeleteReport(elemI, idElem, idUser) {
	...
	ajaxGet(url, function(response) {
		if (JSON.parse(response).length > 0 && response !== 'false') {
			...
			rows.forEach(row => {
				...
				let tdSeeReportsFromElem = document.createElement('td');
				...
				tdSeeReportsFromElem.appendChild(elemLinkElem);
				elemTr.appendChild(tdName);
				elemTr.appendChild(tdContent);
				elemTr.appendChild(tdSeeReportsFromElem);
